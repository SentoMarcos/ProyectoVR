...
public class RoadFromTargetsSticky : MonoBehaviour
{
    ...
    [Tooltip("Cuando un punto alcanza estabilidad por primera vez, deja de actualizar su proxy (mantiene fija la forma).")]
    public bool freezeProxyAfterFirstStable = false;
    [Tooltip("Cuando el plano queda bloqueado (tras suficientes puntos), no se regenera la malla de carretera.")]
    ...
    void RenderOrCache(List<Transform> controlPoints)
    {
        ...
        if (freezeRoadAfterPlaneLocked && planeLocked)
        {
            return;
        }
    ...
    void UpdateProxiesSticky()
    {
        foreach (var st in proxies)
        {
            ...
            if (isVisible)
            {
                ...
                {
                    if (freezeProxyAfterFirstStable)
                    {
                        // Mantener pose estable y no actualizar más
                        continue;
                    }
